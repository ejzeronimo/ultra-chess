...
def reset_board(with_pieces=True):
    def generate_pieces(colour):
        return [pieces.Angel(colour), pieces.Cardinal(colour), pieces.Bishop(colour), pieces.Queen(colour),
    ...
    while True:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.QUIT:
                pg.quit()
                sys.exit()
        draw_text(screen, micro_font, turn, COLOUR, check, playing, promotion, False)
